...
class TestDataset(Dataset):
    ...
    def __getitem__(self, pat_idx):
        ...
        sample = {'id': query_img_path}

        idx = query_lbl.sum(axis=(1, 2)) > 0
        sample['query_image'] = torch.from_numpy(query_img[idx])
        sample['query_label'] = torch.from_numpy(query_lbl[idx])
    ...
    def getSupport(self, label=None, all_slices=True, N=None):
        ...
        sup_img = []
        sup_lbl = []
        for sup_pat_idx in range(len(self.support_dir)):
            img_path = self.support_dir[sup_pat_idx]
            ...
            if all_slices:
                ...
                sup_img.append(img[idx][idx_])
                sup_lbl.append(lbl[idx][idx_])

        sample['sup_image'] = np.stack([np.stack(sup_img, axis=1)], axis=2)
        sample['sup_label'] = np.stack([np.stack(sup_lbl, axis=1)], axis=2)
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sup_lbl = np.stack([lbl_cls[sample[:self.n_shot * self.n_way]]], axis=1)
        qry_lbl = lbl_cls[sample[self.n_shot * self.n_way:]]
        sup_img = img[sample[:self.n_shot * self.n_way]]
        sup_img = np.stack([np.stack((sup_img, sup_img, sup_img), axis=1)], axis=1)
