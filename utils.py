...
class Scores():
    ...
    def record(self, preds, label):
        ...
        tp = torch.sum((label == 1) * (preds == 1)).cpu().numpy()
        tn = torch.sum((label == 0) * (preds == 0)).cpu().numpy()
        fp = torch.sum((label == 0) * (preds == 1)).cpu().numpy()
        fn = torch.sum((label == 1) * (preds == 0)).cpu().numpy()
...
def cal_fice(preds, label):
    ...
    patient_dice.append(2 * tp / (2 * tp + fp + fn))
    patient_iou.append(tp / (tp + fp + fn))

    print('patient_dice: ', patient_dice)
    print('patient_iou: ', patient_iou)

    return patient_dice, patient_iou
